...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('The lengths of the assignments do not match')
        while len(formals) > 0:
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        args = operands.map(lambda arg: scheme_eval(arg, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        argument_eval = do_lambda_form(Pair(target.second, expressions.second), env)
        env.define(target.first, argument_eval)
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    symbol, expressions = nil, nil
    while len(bindings) > 0:
        start = bindings.first
        check_form(start, 2, 2)
        symbol = Pair(start.first, symbol)
        expressions = Pair(scheme_eval(start.second.first, env), expressions)
        bindings = bindings.second
    check_formals(symbol)
    return env.make_child_frame(symbol, expressions)
